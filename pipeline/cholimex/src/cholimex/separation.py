"""Purpose: Load and run Cholimex proposal and overlap separators.

Inputs: Waveforms, model settings, device, chunk and step parameters.
Outputs: Two separated waveform streams with their sample rate.
"""
...
def _load_exported_module(path: str | Path, device: torch.device):
    """Load an exported PT2 module directly via torch.export.load and retarget device."""
...
def _load_dialoguesidon_models(device: str = "cuda", model_id: str | None = None) -> dict:
    ...
    enforce_offline_mode()
    raw_id = model_id or REPO_ID
    local_target, is_dir = resolve_local_model_path(
        raw_id, env_var="DIALOGUESIDON_MODEL_PATH", default_subpath="DialogueSidon"
    )
    resolved = device if (device != "cuda" or torch.cuda.is_available()) else "cpu"
    cache_key = ("dialoguesidon", str(local_target), resolved)
    with _cache_lock:
        ...
        if is_dir or Path(local_target).is_dir():
            target_dir = assert_local_model_exists(
                local_target, required_files=MODEL_FILES, model_name_hint="DialogueSidon"
            )
            paths = {f: str(target_dir / f) for f in MODEL_FILES}
        else:
            paths = {
        ...
        try:
            ...
            scheduler = DPMSolverMultistepScheduler.from_config(meta["ddpm_config"])
...
def _load_mossformer2_models(device: str = "cuda", model_id: str | None = None) -> dict:
    ...
    enforce_offline_mode()
    raw_id = model_id or REPO_ID_MOSSFORMER2
    local_target, is_dir = resolve_local_model_path(
        raw_id, env_var="MOSSFORMER2_MODEL_PATH", default_subpath="MossFormer2"
    )
    resolved = device if (device != "cuda" or torch.cuda.is_available()) else "cpu"
    cache_key = ("mossformer2", str(local_target), resolved)
    if cache_key in _cache:
        return _cache[cache_key]

    if is_dir or Path(local_target).is_dir():
        checkpoint_dir = assert_local_model_exists(
            local_target, required_files=["last_best_checkpoint.pt"], model_name_hint="MossFormer2"
        )
        checkpoint = str(checkpoint_dir / "last_best_checkpoint.pt")
    else:
        checkpoint = hf_hub_download(repo_id=str(local_target), filename="last_best_checkpoint.pt", local_files_only=True)
...
def _load_sepformer_models(device: str = "cuda", model_id: str | None = None) -> dict:
    enforce_offline_mode()
    raw_id = model_id or REPO_ID_SEPFORMER
    local_target, _ = resolve_local_model_path(
        raw_id, env_var="SEPFORMER_MODEL_PATH", default_subpath="sepformer-wsj02mix"
    )
    resolved = device if (device != "cuda" or torch.cuda.is_available()) else "cpu"
    cache_key = ("sepformer", str(local_target), resolved)
    if cache_key in _cache:
        return _cache[cache_key]
    ...
    model = SepformerSeparation.from_hparams(
        ...
        savedir=str(local_target) if Path(local_target).is_dir() else str(Path.home() / ".cache" / "speechbrain" / str(local_target).replace("/", "_")),
