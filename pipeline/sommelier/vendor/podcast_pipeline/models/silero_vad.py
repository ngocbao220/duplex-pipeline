        ...
        try:
            ...
            def patched_init(self, path_or_bytes, sess_options=None, providers=providers, **kwargs):
                ...
                try:
                    ...
                    original_init(self, path_or_bytes, sess_options=sess_options, providers=['CPUExecutionProvider'], **kwargs)

            onnxruntime.InferenceSession.__init__ = patched_init

            # Check local offline loading first
            try:
                from core.model_utils import is_offline_mode, load_local_silero_vad
                if is_offline_mode() or local:
                    vad_model, get_speech_timestamps = load_local_silero_vad(device=str(device))
