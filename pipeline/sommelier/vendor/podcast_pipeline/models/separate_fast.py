...
# This code is from an Apache-2.0 repository.
        ...
        if device == "cuda":
            try:
                self.model = ort.InferenceSession(
                    args["model_path"], sess_options=opts, providers=["CUDAExecutionProvider", "CPUExecutionProvider"]
                )
            except Exception:
                self.model = ort.InferenceSession(
                    args["model_path"], sess_options=opts, providers=["CPUExecutionProvider"]
                )
        elif device == "cpu":
            self.model = ort.InferenceSession(
                args["model_path"], sess_options=opts, providers=["CPUExecutionProvider"]
