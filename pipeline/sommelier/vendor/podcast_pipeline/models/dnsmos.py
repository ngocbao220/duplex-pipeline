        ...
        if device == "cuda":
            try:
                self.onnx_sess = ort.InferenceSession(
                    primary_model_path, sess_options=opts, providers=["CUDAExecutionProvider", "CPUExecutionProvider"]
                )
            except Exception:
                self.onnx_sess = ort.InferenceSession(
                    primary_model_path, sess_options=opts, providers=["CPUExecutionProvider"]
                )
            print("Using CUDA:", self.onnx_sess.get_providers())
        else:
            self.onnx_sess = ort.InferenceSession(primary_model_path, sess_options=opts, providers=["CPUExecutionProvider"])
