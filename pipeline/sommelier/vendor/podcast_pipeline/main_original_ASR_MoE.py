...
try:
    from models import whisper_asr
...
def _extract_speaker_embedding(
    ...
    # Ensure minimum length for pyannote embedder (at least ~0.5s = 8000 samples at 16kHz)
    min_samples = int(0.5 * target_sr)
    if len(segment) < min_samples:
        segment = np.pad(segment, (0, min_samples - len(segment)), mode="wrap")
...
def constrain_speaker_inventory(segment_list, audio_info, embedder: Inference | None,
    ...
    for speaker in speakers:
        ...
        # Try top candidates (with standard window)
        for segment in candidates[:5]:
            embedding = _extract_speaker_embedding(
                audio_info, segment["start"], segment["end"], embedder=embedder, min_duration=0.2
            )
        ...
        # If still no embedding, try all segments of this speaker without strict min_duration
        if not embeddings:
            for segment in candidates[5:]:
                embedding = _extract_speaker_embedding(
                    audio_info, segment["start"], segment["end"], embedder=embedder, min_duration=0.1
                )
                if embedding is not None:
                    embeddings.append(embedding)
                    break
        ...
        if embeddings:
            ...
            if norm > 0:
                ...
                speakers_without_emb.append(speaker)
        else:
            speakers_without_emb.append(speaker)

    if not centroids:
        logger.warning("Could not extract any speaker embeddings for inventory clustering; falling back to first 2 speakers.")
        top_speakers = speakers[:expected_speakers]
        mapping = {spk: f"SPEAKER_{min(i, expected_speakers - 1):02d}" for i, spk in enumerate(speakers)}
        return [{**segment, "speaker": mapping[segment["speaker"]]} for segment in segment_list]

    from sklearn.cluster import AgglomerativeClustering

    valid_speakers = list(centroids.keys())
    if len(valid_speakers) <= expected_speakers:
        # Each valid speaker gets its own cluster
        labels = list(range(len(valid_speakers)))
    else:
        labels = AgglomerativeClustering(
            n_clusters=expected_speakers, metric="cosine", linkage="average"
        ).fit_predict(np.stack([centroids[s] for s in valid_speakers]))

    label_by_speaker = dict(zip(valid_speakers, labels))

    # If some speakers have no embeddings (e.g. tiny blips <0.1s), assign them to nearest/dominant cluster
    dominant_cluster = 0
    if label_by_speaker:
        # Find which cluster has the most duration
        cluster_durations = {}
        for segment in segment_list:
            spk = segment["speaker"]
            if spk in label_by_speaker:
                c = label_by_speaker[spk]
                cluster_durations[c] = cluster_durations.get(c, 0.0) + (float(segment["end"]) - float(segment["start"]))
        if cluster_durations:
            dominant_cluster = max(cluster_durations, key=cluster_durations.get)

    for speaker in speakers_without_emb:
        logger.warning(f"Assigning low-duration speaker '{speaker}' without embedding to cluster {dominant_cluster}")
        label_by_speaker[speaker] = dominant_cluster

    first_start = {}
    for segment in segment_list:
        label = label_by_speaker.get(segment["speaker"], 0)
    ...
    mapping = {speaker: f"SPEAKER_{ordered_labels.get(label, 0):02d}" for speaker, label in label_by_speaker.items()}
...
def export_pre_asr_result(audio_info, segment_list, save_dir, audio_name, audio_duration,
                          ...
                          step0_time=0.0, step0_rt=0.0,
                          constrain_time=0.0, constrain_rt=0.0):
    ...
    output_data = {
        "metadata": {
            ...
            "step0_preprocess": {"processing_time_seconds": step0_time, "rt_factor": step0_rt},
            ...
            "constrain_speakers": {"processing_time_seconds": constrain_time, "rt_factor": constrain_rt},
            "asr_skipped": True,
...
def main_process(audio_path, save_path=None, audio_name=None,
    ...
    try:
        ...
        step0_time = step0_end - step0_start
        ...
        if args.expected_speakers:
            ...
            constrain_rt = constrain_time / audio_duration if audio_duration > 0 else 0
            logger.info(f"Constrain speakers - Processing time: {constrain_time:.2f}s, RT factor: {constrain_rt:.4f}")
        ...
        if args.until_pre_asr:
            ...
            return export_pre_asr_result(
                ...
                separation_rt, overlap_threshold,
                step0_time=step0_time, step0_rt=step0_rt,
                constrain_time=constrain_time, constrain_rt=constrain_rt,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="300.0",
        help="Maximum chunk duration in seconds for VAD dialogue splitting (default: 300.0 / 5 minutes, or 'full' for whole audio)",
    )
    ...
    try:
        ...
        dia_pipeline = None
    # ASR (Skipped in Sommelier pipeline because --until-pre-asr is used)
    asr_model = None
    if not args.until_pre_asr and whisper_asr is not None:
        ...
        logger.debug(" * Skipping ASR model loading (--until-pre-asr enabled or whisper_asr not installed)")
    if args.ASRMoE:
        import nemo.collections.asr as nemo_asr
        from nemo.collections.speechlm2.models import SALM
        para_path, _ = resolve_local_model_path("nvidia/parakeet-tdt-0.6b-v2", env_var="PARAKEET_MODEL_PATH", default_subpath="parakeet-tdt-0.6b-v2")
        asr_model_2 = nemo_asr.models.ASRModel.from_pretrained(model_name=str(para_path))

        # Load Canary model
        logger.debug(" * Loading Canary Model")
        canary_path, _ = resolve_local_model_path("nvidia/canary-qwen-2.5b", env_var="CANARY_MODEL_PATH", default_subpath="canary-qwen-2.5b")
        canary_model = SALM.from_pretrained(str(canary_path))
