...
class DNSMOSScorer:
    def __init__(self, cfg: Config):
        ...
        try:
            ...
            try:
                self.session = ort.InferenceSession(str(cfg.benchmark_dnsmos_model_path), sess_options=opts, providers=providers)
            except Exception:
                self.session = ort.InferenceSession(str(cfg.benchmark_dnsmos_model_path), sess_options=opts, providers=["CPUExecutionProvider"])
    ...
    def __init__(self, cfg: Config):
        ...
        try:
            ...
            local_target, _ = resolve_local_model_path(
                cfg.benchmark_speaker_embedding_model,
